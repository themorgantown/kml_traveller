...
if not ORS_API_KEY:
    raise ValueError("ORS_API_KEY not found in environment variables. Please check your .env file.")
...
def geocode_addresses(addresses, location_names, client, cache):
	coordinates = []
	final_names = []
	...
	for i, address in enumerate(addresses):
		...
		if cached_result:
			if isinstance(cached_result, dict): # Handle old cache format from run.py
				lat, lon = cached_result['lat'], cached_result['lng']
			else: # Handle new cache format
		...
		try:
			...
			if geocode_result and geocode_result['features']:
				# ORS returns [lon, lat] format
				lon, lat = geocode_result['features'][0]['geometry']['coordinates']
				coordinates.append((lat, lon))
				final_names.append(name)
				
				# Cache the result
				cache.set_geocode(address, (lat, lon))
				print(f"  {len(coordinates)}. {name}: ({lat:.6f}, {lon:.6f})")
			else:
...
def create_distance_matrix(locations, client, cache):
	n = len(locations)
	...
	if centroid:
		avg_distance_from_center = sum(
			haversine_distance(lat, lon, centroid[0], centroid[1]) 
	...
	print(f"Processing {len(pairs_to_calculate)} distance calculations...")
	
	# ORS allows matrix requests up to 50x50
	# We'll send all pairs in one go if possible
	if n <= 50:
		try:
			# ORS expects (lon, lat)
			...
			for i in range(n):
				for j in range(n):
					if i != j:
						...
						# Cache the result
						cache.set_distance(locations[i], locations[j], int(distance))
			
			print("Distance matrix created successfully using a single API call!")
			return matrix

		except Exception as e:
			print(f"Error in batch distance matrix request: {str(e)}")
			print("Falling back to individual requests...")
	
	# Fallback to individual requests if matrix API fails or for > 50 locations
	for i, j in pairs_to_calculate:
		...
		try:
			# ORS expects (lon, lat)
			ors_origin = [origin[1], origin[0]]
			ors_destination = [destination[1], destination[0]]
			
			routes = client.directions(
				coordinates=[ors_origin, ors_destination],
				profile='driving-car',
				format='json'
			)
			...
			if routes and 'routes' in routes and routes['routes']:
				...
				matrix[i][j] = int(distance)
				matrix[j][i] = int(distance)  # Symmetric
				
				# Cache the result
				cache.set_distance(origin, destination, int(distance))
			else:
				# Use geographic distance as fallback
				fallback_distance = int(haversine_distance(
					origin[0], origin[1], destination[0], destination[1]
				) * 1.5)
				matrix[i][j] = fallback_distance
				matrix[j][i] = fallback_distance
			
			time.sleep(0.1) # Rate limit
			
		except Exception as e:
			print(f"Error calculating distance between {i} and {j}: {str(e)}")
			# Use geographic distance as fallback
			fallback_distance = int(haversine_distance(
				origin[0], origin[1], destination[0], destination[1]
			) * 1.5)
...
def solve_tsp_with_clustering(matrix, locations, max_cluster_size=50):
	...
	for i, cluster in enumerate(clusters):
		...
		locations_in_cluster = [locations[idx] for idx in cluster]
		
		# Solve TSP for this cluster
		cluster_route = solve_tsp(cluster_matrix, locations_in_cluster)
		...
		cluster_center_idx = find_optimal_start_location(locations_in_cluster)
...
def save_kml(locations, route, location_names=None, output_file="optimized_route.kml"):
	...
	kml_content.append(os.linesep.join(path_coords))
	kml_content.append('        </coordinates>')
	kml_content.append('      </LineString>')
	kml_content.append('    </Placemark>')
	...
	with open(output_file, 'w', encoding='utf-8') as f:
		f.write(os.linesep.join(kml_content))
...
Step 2: Converting {len(addresses)} addresses to GPS coordinates...")
	try:
		...
		coords, final_location_names = geocode_addresses(addresses, location_names, client, cache)
	except Exception as e:
...
Step 3: Creating distance matrix for {len(coords)} locations...")
	try:
		distance_matrix = create_distance_matrix(coords, client, cache)
	except Exception as e:
